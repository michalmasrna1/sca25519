...
void fe25519_cmov(fe25519* inoutTarget, const fe25519* in, int condition);

void fe25519_cswap(fe25519* in1, fe25519* in2, int condition);

void fe25519_setone(fe25519* out);

void fe25519_setzero(fe25519* out);
...
int32_t fe25519_iseq_vartime(fe25519* in1, fe25519* in2);

// WARNING: This function uses negative logic (0 if zero)
int32_t fe25519_iszero(fe25519* in);

int32_t fe25519_getparity(fe25519* in);

#if (defined(__clang__) || defined(__GNUC__)) && defined(CORTEX_M4)

inline void fe25519_sub(fe25519* out, const fe25519* baseValue,
...
inline void fe25519_sub(fe25519* out, const fe25519* baseValue,
  ...
  asm volatile(
      ...
      "adds %[cR3],%[cR1],%[cR1] \n\t"  // double word #7 and #8, keep the
                                        // un-doubled word #7 in cR1
      "adcs %[cR2],%[cR2],%[cR2] \n\t"

      "orr %[cR1],%[cR1],#(1 << 31) \n\t"  // set bit #31

      "STR %[cR1], [%[out],#28] \n\t"  // temporarily store the value for word
                                       // #7

      "sub %[cR1],%[cR2],#1 \n\t"

      "mov %[cR2],#(-19) \n\t"
      "mul %[cR1],%[cR1],%[cR2] \n\t"  // negative reduction value for result
                                       // word #0

      "LDR %[cR2], [%[valueToSubstract],#0] \n\t"
      "LDR %[cR3], [%[baseValue],#0] \n\t"
      "umaal %[cR2],%[cR1],%[zero],%[zero]\n\t"  // accumulate values to
                                                 // subtract.
      ...
      : [ cR1 ] "=r"(clobberReg1), [ cR2 ] "=r"(clobberReg2),
        [ cR3 ] "=r"(clobberReg3), [ out ] "+r"(out),
        [ baseValue ] "+r"(baseValue),
        [ valueToSubstract ] "+r"(valueToSubstract), [ zero ] "+r"(zero)
      :
      : "memory", "cc");
}
...
void fe25519_sub(fe25519* out, const fe25519* baseValue,
                 const fe25519* valueToSubstract);

#endif

void fe25519_neg(fe25519* out, const fe25519* valueToNegate);

#if (defined(__clang__) || defined(__GNUC__)) && defined(CORTEX_M4)

inline void fe25519_add(fe25519* out, const fe25519* baseValue,
                        const fe25519* valueToSubstract)
    __attribute__((always_inline));
...
inline void fe25519_add(fe25519* out, const fe25519* baseValue,
  ...
  asm volatile(
      ...
      : [ cR1 ] "=r"(clobberReg1), [ cR2 ] "=r"(clobberReg2),
        [ cR3 ] "=r"(clobberReg3), [ cR4 ] "=r"(clobberReg4), [ out ] "+r"(out),
        [ baseValue ] "+r"(baseValue), [ valueToAdd ] "+r"(valueToAdd),
        [ one ] "+r"(one)
      :
      : "memory", "cc");
}
...
void fe25519_add_asm(fe25519* out, const fe25519* in1, const fe25519* in2);

#define fe25519_add fe25519_add_asm

#else  // #ifdef CRYPTO_HAS_ASM_FE25519_ADD

void fe25519_add(fe25519* out, const fe25519* in1, const fe25519* in2);

#endif  // #else #ifdef CRYPTO_HAS_ASM_FE25519_ADD
#endif  //#else #if (defined(__clang__) || defined(__GNUC__)) && defined
        //(CORTEX_M4)

#ifdef CRYPTO_HAS_ASM_FE25519_MPY121666

/// Note that out and in are allowed to overlap!
void fe25519_mpyWith121666_asm(fe25519* out, const fe25519* in);
#define fe25519_mpyWith121666 fe25519_mpyWith121666_asm

#else

/// Note that out and in are allowed to overlap!
void fe25519_mpyWith121666(fe25519* out, const fe25519* in);
#endif

#if (defined(__clang__) || defined(__GNUC__)) && defined(CORTEX_M4)

inline void fe25519_mpy121666add(fe25519* out, const fe25519* valueToAdd,
...
inline void fe25519_mpy121666add(fe25519* out, const fe25519* valueToAdd,
  ...
  asm volatile(
      ...
      : [ cR1 ] "=r"(clobberReg1), [ cR2 ] "=r"(clobberReg2),
        [ cR3 ] "=r"(clobberReg3), [ cR4 ] "=r"(clobberReg4), [ out ] "+r"(out),
        [ valueToMpy ] "+r"(valueToMpy), [ valueToAdd ] "+r"(valueToAdd),
        [ v121666 ] "+r"(v121666)
      :
      : "memory", "cc");
}
...
extern void fe25519_reduceTo256Bits_asm(fe25519* res, const UN_512bitValue* in);

#endif

/// Used for fast randomization of field elements. Use 16 bit randomization
/// constant since it's easy and fast to implement and it's repeated application
/// is still considered to make statistical analysis very hard.
void fe25519_mpyWith_uint16(fe25519* inOut, const uint16_t valU16);

/// Used for fast randomization of field elements. Use 31 bit randomization
/// constant since it's easy and fast to implement and it's repeated application
/// is still considered to make statistical analysis very hard. Note that bit
/// #31 is implicitly cleared. For a zero multiplier, the return value is
/// undefined! This allows for some optimizations and does not actually generate
/// any problem when using the function for projective randomization.
// void
...
inline void fe25519_mpyWith_uint31(fe25519* inOut, uint32_t mpyVal)
    __attribute__((always_inline));
...
inline void fe25519_mpyWith_uint31(fe25519* inOut, uint32_t mpyVal) {
  ...
  asm volatile(
      ...
      "lsr %[cR1],%[cR1],#1\n\t"  // Shift back the result word. cR1 holds now
                                  // result word #7

      "lsr %[mv],%[mv],#1\n\t"  // Shift back the multiplicand value. This way
                                // also bit #31 is cleared.
      "sub %[mv],%[mv],#1\n\t"  // Subtract one from the multiplicand value.

      "mov %[cR3], #19 \n\t"
      "umull %[cR2],%[cR3],%[cR3],%[cR2]\n\t"  // cR2 now holds word #0 and cR3
                                               // word #1 of the result word
                                               // component stemming from
                                               // reduction.

      "ldr %[cR4],[%[io],#(0*4)] \n\t"        // load word #0.
      "umaal %[cR4],%[cR2],%[cR4],%[mv]\n\t"  // multiply word
      "add %[cR2],%[cR2],%[cR3]\n\t"  // add the remaining reduction word #1. We
                                      // know that cR3 is smaller than 19 and
                                      // cR2 has bit #31 cleared. No overflow
                                      // possible.
      "str %[cR4],[%[io],#(0*4)] \n\t"  // store word
      ...
      : [ cR1 ] "=r"(clobberReg1), [ cR2 ] "=r"(clobberReg2),
        [ cR3 ] "=r"(clobberReg3), [ cR4 ] "=r"(clobberReg4),
        [ io ] "+r"(inOut), [ mv ] "+r"(mpyVal)
      :
      : "memory", "cc");
}
...
void fe25519_unpack(fe25519* out, const uint8_t in[32]);
...
void fe25519_mul_asm(fe25519* result, const fe25519* in1, const fe25519* in2);

#define fe25519_mul fe25519_mul_asm

#else

void fe25519_mul(fe25519* result, const fe25519* in1, const fe25519* in2);

#endif

#ifdef CRYPTO_HAS_ASM_FE25519_SQUARE

void fe25519_square_asm(fe25519* result, const fe25519* in);
#define fe25519_square fe25519_square_asm

#else
void fe25519_square(fe25519* result, const fe25519* in);
...
void fe25519_invert_useProvidedScratchBuffers(fe25519* r, const fe25519* x,
                                              fe25519* t1, fe25519* t2,
                                              fe25519* t3);

void fe25519_pow2523_useProvidedScratchBuffers(fe25519* r, const fe25519* x,
                                               fe25519* t1, fe25519* t2,
                                               fe25519* t3);

void fe25519_elligator2x_useProvidedScratchBuffers(fe25519* x, const fe25519* r,
                                                   // Scratch buffers
                                                   fe25519* t0, fe25519* t1,
                                                   fe25519* t2, fe25519* t3,
                                                   fe25519* v);

void fe25519_generateRandomValue(fe25519* result);

void fe25519_elligator2(fe25519* x, const fe25519* r);

void fe25519_elligator2_useProvidedScratchBuffers(fe25519* x, const fe25519* r,
                                                  // Scratch buffers
                                                  fe25519* t0, fe25519* t1,
                                                  fe25519* t2, fe25519* t3,
                                                  fe25519* v);

int fe25519_squareroot(fe25519* result, const fe25519* in);
