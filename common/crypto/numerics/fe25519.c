...
/// We are already using a packed radix 16 representation for fe25519. The real
/// use for this function is for architectures that use more bits for storing a
/// fe25519 in a representation where multiplication may be calculated more
/// efficiently. Here we simply copy the data.
...
void fe25519_cmov(fe25519* result, const fe25519* in, int condition) {
...
void fe25519_sub(fe25519* out, const fe25519* baseValue,
  ...
  // We always set bit #31, and compensate this by subtracting 1 from the
  // reduction value.
...
void fe25519_neg(fe25519* out, const fe25519* valueToNegate) {
  ...
  // We always set bit #31, and compensate this by substracting 1 from the
  // reduction value.
  out->as_uint32_t[7] = ((uint32_t)accu) | 0x80000000ul;
...
void fe25519_add(fe25519* out, const fe25519* baseValue,
...
void fe25519_mpyWith121666(fe25519* out, const fe25519* in) {
  ...
  // Unfortunately, this value does not fit completely in one single 16 bit
  // word. We split the operation to one 16 bit multiply and one "<<16 and add"
  // operation.
...
static void fe25519_reduceTo256Bits(fe25519* res, const UN_512bitValue* in) {
  ...
  // Now let's reduce bit #31 of word 31 and possibly the remnants of the
  // reduction of word #15. As a consequence at most 38 may be added to word #7
  // within the next reduction steps. Since bit #31 of word #7 is already
  // reduced in the step to come, we may not get any overflow
  accu = ((uint32_t)(accu >> 31)) * 19;
  {
...
int fe25519_squareroot(fe25519* result, const fe25519* in) {
  int retval = -1;
  fe25519 d, b, one;
  fe25519 pminusone = {{0xec, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
  ...
  if (fe25519_iseq_vartime(&b, &one)) {
    ...
    retval = 0;
  } else if (fe25519_iseq_vartime(&b, &pminusone)) {
    // b=p-1
    // r=2a(4a)^((p-5)/8)
    fe25519_add(&r, in, in);
